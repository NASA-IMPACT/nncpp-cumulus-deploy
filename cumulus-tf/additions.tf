...
resource "aws_lambda_function" "generate_and_save_granule_metadata" {
  function_name = "${var.prefix}-GenerateAndSaveGranuleMetadata"
  filename      = data.archive_file.cmr.output_path
  role          = module.cumulus.lambda_processing_role_arn
  handler       = "index.generateAndSaveGranuleMetadataHandler"
  runtime       = "nodejs12.x"
  timeout       = 900
  memory_size   = 2048
...
module "generate_and_save_granule_metadata_workflow" {
  source = "https://github.com/nasa/cumulus/releases/download/v8.1.0/terraform-aws-cumulus-workflow.zip"

  prefix          = var.prefix
  name            = "GenerateAndSaveGranuleMetadataWorkflow"
  workflow_config = module.cumulus.workflow_config
  system_bucket   = var.system_bucket
  tags            = local.tags

  state_machine_definition = templatefile(
    "${path.module}/generate_and_save_granule_metadata.asl.json",
    {
      generate_and_save_granule_metadata_task_arn: aws_lambda_function.generate_and_save_granule_metadata.arn
    }
  )
}