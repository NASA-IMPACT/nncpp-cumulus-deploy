...
def generate_and_upload_cog(granule):
    ...
    with rasterio.Env(**gdal_config):
        ...
        # Write Hdf data to an in memory geotiff, this will be the source 'file' for cog_translate
        with MemoryFile() as memfile_src:

            with memfile_src.open(**output_profile) as mem_geotiff:
                
                # Add bands arrays to dataset writer
                mem_geotiff.write(np.stack(bands))
        
                for idx, variable_name in enumerate(modis_config["variable_names"], 1):
                    mem_geotiff.set_band_description(idx, variable_name)
                
                del bands

                # Open a destination memory file for the COG generated by cog_translate(src=mem_geotiff, dst=memfile_dst)
                with MemoryFile() as memfile_dst:
                    ...
                    assert cog_validate(memfile_dst.name)[0]

                    # Describe the memory file in order to verify the upload
                    memfile_md5 = md5_digest_memfile(memfile_dst)
                    memfile_etag = compute_memfile_etag(memfile_dst, DEFAULT_CHUNKSIZE)
                    ...
                    print(f"Uploaded {memfile_dst.name} to {output_s3_path}")
